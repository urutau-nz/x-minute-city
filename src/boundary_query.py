...
def main(config, id_city):
    ...
    # for id_city in np.arange(0,500,1):
        # query the distances
    origxdest = query_points(db, config, id_city)
        # add df to sql
    write_to_postgres(origxdest, db, id_city)
        # determine nearest
...
############## Query Points ##############
def query_points(db, config, id_city):
    '''
    query OSRM for distances between origins and destinations
    '''
    logger.error('Querying invoked for {} in city #{}'.format(config['transport_mode'], id_city))
    location = config['location']
    # connect to db
    cursor = db['con'].cursor()

    # get list of all origin ids that have a population > 0
    sql = '''SELECT geoid, st_x(centroid) as x, st_y(centroid) as y FROM origin_{} WHERE "U7B001">0;'''.format(id_city)
    ...
    sql = "SELECT id_dest, dest_type, st_x(geometry) as lon, st_y(geometry) as lat FROM destination_{} WHERE dest_type IN ('{}') ;".format(id_city, "','".join(config['services']))
    ...
    if config['query_euclidean']:
        ...
        origxdest = euclidean_query(origxdest, orig_df, dest_df, config, id_city)
        # subset
        # code.interact(local=locals())
        origxdest = origxdest.rename_axis('id_orig').reset_index()
        origxdest = origxdest[['id_orig', 'id_dest']+config['metric']+['dest_type']]
    else:
    ...
    origxdest[config['metric']] = origxdest[config['metric']].apply(pd.to_numeric, downcast='integer')
    return origxdest
...
def euclidean_subset_sql(db, config, id_city, origxdest_size):
    # identify pairs to query based on a subset
    distance_threshold = 2000
    dests_number = 5
    conn = db['engine'].raw_connection()
    cur = conn.cursor()
    if origxdest_size < 1e8:
        # # calculate the euclidean distance of the pairs
        queries = ['DROP TABLE IF EXISTS origxdest;',
                '''CREATE TABLE origxdest
                AS 
                SELECT origins20.geoid as id_orig, destinations.id_dest as id_dest, destinations.dest_type, st_distancesphere(origins20.centroid, destinations.geometry)::int as euclidean
                FROM origins20 CROSS JOIN destinationsdocker
                WHERE origins20.id_city={} 
                AND destinations.id_city={} 
                AND origins20."U7B001">0
                AND destinations.dest_type IN ('{}')
                '''.format(id_city, id_city, "','".join(config['services']))
        ]
        for q in queries:
            cur.execute(q)
        conn.commit()
        logger.error('Euclidean distances calculated')
        # get all values within the distance_threshold
        sql = '''SELECT id_orig, id_dest, dest_type FROM origxdest WHERE euclidean<{}'''.format(distance_threshold)
        pairs = pd.read_sql(sql, db['con'])
        logger.error('Subset within distance {}'.format(distance_threshold))
        # get closest n pairs
        sql = '''SELECT rank_filter.id_orig, rank_filter.id_dest, rank_filter.dest_type FROM (
                SELECT origxdest.*, 
            rank() OVER (
                PARTITION BY id_orig, dest_type
                ORDER BY euclidean ASC
            )
            FROM origxdest
            ) rank_filter WHERE RANK <= {}'''.format(dests_number)
        pairs_additional = pd.read_sql(sql, db['con'])
        # import code
        # code.interact(local=locals())
        logger.error('Determined closest {} destinations for the remaining origins'.format(dests_number))
        # merge
        ...
        logger.error('Large number of pairs, subsetting by iterating through origins')
        # get list of all origin ids that have a population > 0
        sql = '''SELECT distinct(geoid) as id_orig FROM origins20 WHERE id_city={} AND "U7B001">0;'''.format(id_city)
        id_origs = pd.read_sql(sql, db['con'])['id_orig'].values
        # code.interact(local=locals())
        dfs = []
        for idx in tqdm(id_origs):
            queries = ['DROP TABLE IF EXISTS origxdest;',
                    ...
                    AS 
                    SELECT origins20.geoid as id_orig, destinations.id_dest as id_dest, destinations.dest_type, st_distancesphere(origins20.centroid, destinations.geometry)::int as euclidean
                    FROM origins20 CROSS JOIN destinations
                    WHERE origins20.geoid='{}' 
                    AND destinations.id_city={} 
                    AND origins20."U7B001">0
                    AND destinations.dest_type IN ('{}')
                    '''.format(idx, id_city, "','".join(config['services']))
                    ]
            for q in queries:
                cur.execute(q)
            conn.commit()
...
def euclidean_query(origxdest, orig_df, dest_df, config, id_city):
...
def req(query_string, config):
    time.sleep(0.05)
    response = requests_retry_session(retries=10, backoff_factor=0.3).get(query_string).json()
    if len(config['metric']) == 2:
...
############## Save to SQL ##############
def write_to_postgres(df, db, id_city, indices=True):

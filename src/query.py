...
############## Query Points ##############
def query_points(db, config, transport_mode):
    '''
    ...
    sql = '''SELECT "{}", st_x(geom) as x, st_y(geom) as y FROM origin WHERE "C18_CURPop">0;'''.format(config['orig_id'])
    orig_df = pd.read_sql(sql, db['con'])

    # drop duplicates
    orig_df.drop_duplicates(inplace=True)
    # set index (different format for different census blocks)
    orig_df['orig_id'] = orig_df[config['orig_id']].astype('category')
    orig_df = orig_df.set_index('orig_id')
    ...
    if config['query_euclidean']:
        ...
        origxdest = euclidean_query(origxdest, orig_df, dest_df, config, transport_mode)
        # subset
        ...
        for metric in config['metric']:
            origxdest['{}'.format(metric)] = None
        origxdest = execute_table_query(origxdest, orig_df, dest_df, config, transport_mode)

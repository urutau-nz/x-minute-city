...
def main(config_filename=None):
    ...
    if config_filename is None:
        config_filename = 'main'#input('Insert Config Filename (filename.yaml): ')
        if ('yaml' in config_filename) == True:
            config_filename = config_filename[:-5]

    # import config file
    with open('./src/boundary_{}.yaml'.format(config_filename)) as file:
    ...
    # destinations: separate files
    # origins: get_blocks.py

    # query
    boundary_query.main(config)
